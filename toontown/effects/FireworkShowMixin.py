...
class FireworkShowMixin:
    ...
    def startMusic(self):
        if self.timestamp:
            ...
            base.audioPlaybackManager.playSong(self.showMusic, 0, 1, 0.8, max(0, t))
    ...
    def preShow(self, eventId, startT):
        ...
        if self.__checkHoodValidity() and hasattr(base.cr.playGame, 'hood') and base.cr.playGame.hood and hasattr(base.cr.playGame.hood, 'sky') and base.cr.playGame.hood.sky:
            preShow = Sequence(Func(base.localAvatar.setSystemMessage, 0, startMessage), Parallel(LerpColorScaleInterval(base.cr.playGame.hood.sky, 2.5, Vec4(0.0, 0.0, 0.0, 1.0)), LerpColorScaleInterval(base.cr.playGame.hood.loader.geom, 2.5, Vec4(0.25, 0.25, 0.35, 1)), LerpColorScaleInterval(base.localAvatar, 2.5, Vec4(0.85, 0.85, 0.85, 1)), Func(__lightDecorationOn__)), Func(base.setBackgroundColor, Vec4(0, 0, 0, 1)), Func(self.__checkDDFog), Func(base.camLens.setFar, 1000.0), Func(base.cr.playGame.hood.sky.hide), Func(base.localAvatar.setSystemMessage, 0, instructionMessage), Func(self.getLoader().music.stop), Wait(2.0), Func(base.audioPlaybackManager.playSong, self.showMusic, 0, 1, 0.8, max(0, startT)))
            return preShow
        return None
    ...
    def postShow(self, eventId):
        ...
        if self.restorePlaygroundMusic:
            ...
            postShow.append(Func(base.audioPlaybackManager.playSong, self.getLoader().music, 1, 1, 0.8))
        return postShow
